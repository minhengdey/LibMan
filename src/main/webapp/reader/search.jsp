                ...
                <div class="card">
                    ...
                    <div id="results">
                        <% List<Document> allDocuments = (List<Document>) request.getAttribute("documents");
                                ...
                                if (currentPage < 1) currentPage=1; } } catch (NumberFormatException e) { currentPage=1;
                                    } int totalDocuments=(allDocuments !=null) ? allDocuments.size() : 0; int
                                    totalPages=(int) Math.ceil((double) totalDocuments / pageSize); if (currentPage>
                                    ...
                                    <div style="margin-bottom: 0.3rem; color: var(--text-light);">
                                        Tìm thấy <strong>
                                            <%= totalDocuments %>
                                        </strong> kết quả cho "<strong>
                                            <%= keyword %>
                                        </strong>"
                                    </div>
                                    <% } if (allDocuments !=null && !allDocuments.isEmpty()) { %>
                                        <div class="document-grid">
                                            <% for (int i=startIndex; i < endIndex; i++) { Document
                                                doc=allDocuments.get(i); %>
                                        ...
                                        <% if (totalPages> 1) { %>
                                            <div class="pagination">
                                                <% String baseUrl=request.getContextPath() + "/reader/search" ; if
                                                    (keyword !=null && !keyword.trim().isEmpty()) { baseUrl
                                                    +="?keyword=" + java.net.URLEncoder.encode(keyword, "UTF-8" ); }
                                                    else { baseUrl +="?keyword=" ; } %>
                                            ...
                                            <% } %>
                                                <% } else if (keyword !=null && !keyword.trim().isEmpty()) { %>
                                                    <div class="alert alert-info">Không tìm thấy kết quả nào cho từ khóa
                                                        "<strong>
                                                            <%= keyword %>
                                                        </strong>"</div>
                                                    <% } %>
